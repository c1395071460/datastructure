...
//        根据前面的思路 完成表达式的运算
        String express = "7*2*2-5+1-5+3-4"; //如何处理多位数的问题
//        创建两个栈  数栈和符号栈
...
//                        然后将当前的操作符入符号栈
                ...
                if (index == express.length() - 1) {
                    ...
                    //判断下一个字符是否是数字 如果是数字 则继续扫描 如果是运算符 则入栈
                    //注意看是后一位 不是index++
...
//        当表达式扫描完毕 就顺序的从 数栈和读好站中pop出相应的数和符号 并运行
        while (true) {
...
class ArrayStack2 {
    ...
    private int[] stack; //数组  数组模拟战  数据就放在该数组
