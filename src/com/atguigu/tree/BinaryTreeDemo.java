...
public class BinaryTreeDemo {
    public static void main(String[] args) {
        ...
        HeroNode infixNode = binaryTree.infixOrderSearch(no);
        if (resNode != null) {
            System.out.printf("找到了，信息为no = %d name=%s", infixNode.getNo(), infixNode.getName());
        } else {
            System.out.printf("没有找到no = %d 的英雄", no);
        }
        ...
        //后序遍历
        System.out.println("前序遍历查找方式~~~");
        HeroNode postNode = binaryTree.postOrderSearch(no);
        if (resNode != null) {
            System.out.printf("找到了，信息为no = %d name=%s", postNode.getNo(), postNode.getName());
        } else {
            System.out.printf("没有找到no = %d 的英雄", no);
        }
        */
...
class HeroNode {
    ...
    public void delNode(int no) {
        /*
        思路：
            1.因为我们的二叉树是单向的，所以我们是判断房钱节点的子节点是否需要删除节点，而不能去判断当前这个节点是不是需要删除节点，
